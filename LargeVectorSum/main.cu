...
__global__ void
largeVectorAdd(const int *a, int const *b, int *c, size_t vec_size_) {
    const auto tId = blockIdx.x * blockDim.x + threadIdx.x;
    if (tId < vec_size_) c[tId] = a[tId] + b[tId];
}
...
main() {
    ...
    const auto NUM_DATA = 200'000'000;
    ...
    const auto thread_x_per_blk = 1024;
    const auto dimGrid  = dim3(ceil(float(NUM_DATA) / thread_x_per_blk), 1, 1);
    const auto dimBlock = dim3(thread_x_per_blk, 1, 1);

    timer.tick(timer_kernel_exec);
    largeVectorAdd<<<dimGrid, dimBlock>>>(da, db, dc, NUM_DATA);
